...
@api_view(['POST'])
# @permission_classes([IsAuthenticated]) # Uncomment when auth is strict
...
"""
    ...
    remote_path = "/opt/openehrcore/backend-django/fhir_api/views_consent.py"
    
    remote_script = f"""
with open('{remote_path}', 'w', encoding='utf-8') as f:
    f.write({repr(content)})
print("Created views_consent.py")
"""
    client.exec_command("echo \"" + remote_script.replace('"', '\\"') + "\" > /tmp/create_cv.py")
    client.exec_command("python3 /tmp/create_cv.py")
