...
def get_patient_summary(request, patient_id):
    ...
    if not validate_patient_id(patient_id):
        return Response(
            {"error": "Invalid patient ID format"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    immunizations = fetch_safe(
        "Immunization", {"patient": patient_id, "_count": "20", "_sort": "-date"}
    )
    diagnostic_reports = fetch_safe(
        "DiagnosticReport", {"patient": patient_id, "_count": "10", "_sort": "-date"}
    )
    appointments = fetch_safe(
        "Appointment", {"patient": patient_id, "_count": "10", "_sort": "-date"}
    )
    ...
    try:
        ...
        log_ai_inference(
            request.user, 'patient_summary', patient_id=patient_id, grounded=using_ai
        )
        ...
        return Response(
            {
                "error": "Falha ao gerar resumo clínico",
                "using_ai": False,
                "timestamp": datetime.now().isoformat(),
            },
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def check_interactions(request):
    ...
    try:
        ...
        if patient_id:
            if not validate_patient_id(patient_id):
                return Response(
                    {"error": "Invalid patient_id"}, status=status.HTTP_400_BAD_REQUEST
                )
