...
def listar_familias_cbo(request):
    ...
    return Response(
        {
            "count": len(familias),
            "results": [{"codigo": k, "nome": v} for k, v in familias.items()],
        }
    )
...
def validar_cbo(request):
    ...
    if not codigo:
        return Response(
            {"error": "Código não informado"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    return Response(
        {
            ...
            "count": len(ocupacoes),
            "especialidades": [
                {"codigo": o.codigo, "nome": o.nome, "descricao": o.descricao}
                for o in ocupacoes
            ],
        }
    )
