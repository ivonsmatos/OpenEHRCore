...
def subscribe(request):
    ...
    if not topic:
        return Response(
            {"error": "hub.topic is required"}, status=status.HTTP_400_BAD_REQUEST
        )

    subscriber_id = str(
        request.data.get(
            "subscriber_id",
            request.user.user_id if hasattr(request.user, "user_id") else "subscriber",
        )
    )

    hub = get_fhircast_hub()
    result = hub.subscribe(
        topic=topic, subscriber_id=subscriber_id, callback_url=callback, events=events
    )
...
def publish_event(request, topic):
    ...
    if not event_type:
        return Response(
            {"error": "hub.event is required"}, status=status.HTTP_400_BAD_REQUEST
        )
...
def get_context(request, topic):
    ...
    return Response({"hub.topic": f"fhircast/{topic}", "context": context})
...
def get_history(request, topic):
    ...
    return Response(
        {"hub.topic": f"fhircast/{topic}", "events": history, "count": len(history)}
    )
