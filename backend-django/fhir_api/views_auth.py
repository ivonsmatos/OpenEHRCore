...
def health_check(request):
    ...
    try:
        ...
        return Response(
            {"status": "error", "fhir_server": "unhealthy", "error": str(e)},
...
def login(request):
    ...
    return Response(
        {"access_token": token, "token_type": "Bearer"}, status=status.HTTP_200_OK
    )
...
def manage_patients(request):
    ...
    if request.method == "GET":
        try:
            ...
            data = fhir_service.search_patients(
                name=request.query_params.get("name"), offset=offset, count=page_size
            )
            ...
            return JsonResponse(
                {"error": "Erro interno ao listar pacientes", "detail": str(e)},
                status=500,
            )
    ...
    # 2. CRIAR PACIENTE (POST)
        ...
        try:
            ...
            logger.info(
                f"Criando paciente. Usuário: {user_info.get('preferred_username')}"
            )
...
def search_patients_advanced(request):
    ...
    try:
        ...
        traceback.print_exc()
        return Response(
            {"error": "Erro interno ao buscar pacientes", "detail": str(e)},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
def get_patient(request, patient_id):
    ...
    try:
        user_info = request.user
        logger.info(
            f"Acessando paciente {patient_id}. Método: {request.method}. Usuário: {user_info.get('preferred_username')}"
        )
        ...
        if request.method == "GET":
            ...
            updated_patient = fhir_service.update_patient_resource(
...
def get_encounters(request, patient_id):
    ...
    try:
        ...
        return Response(
            {"error": "Erro ao recuperar encontros"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
def get_observations(request, patient_id):
    ...
    try:
        ...
        fhir_service = FHIRService(request.user)
        observations = fhir_service.get_observations_by_patient_id(
        ...
        return Response(
            {"error": "Erro ao recuperar observações"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
def get_conditions(request, patient_id):
    try:
        ...
        return Response(
            {"error": "Erro ao recuperar condições"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
def get_allergies(request, patient_id):
    try:
        ...
        return Response(
            {"error": "Erro ao recuperar alergias"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
def create_service_request(request):
    try:
        ...
        return Response(
            {"error": "Erro ao criar solicitação de exame"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
def create_clinical_impression(request):
    ...
    try:
        ...
        return Response(
            {"error": f"Erro ao criar nota de evolução: {str(e)}"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
