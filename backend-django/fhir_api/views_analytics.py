...
def get_population_metrics(request):
    ...
    try:
        ...
        return JsonResponse(
            {"error": "Failed to generate analytics", "detail": str(e)}, status=500
        )
...
def generate_analytics_report(request):
    ...
    try:
        ...
        if len(clin_table_data) > 1:
            ...
            elements.append(
                Paragraph("Nenhum dado clínico disponível.", styles["Normal"])
            )
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="relatorio_gerencial_{datetime.now().strftime("%Y%m%d")}.pdf"'
        )
        response.write(pdf)
        return response
