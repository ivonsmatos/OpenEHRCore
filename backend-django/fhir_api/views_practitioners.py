    ...
    try:
            ...
            "name": [
                {
                    ...
                    "given": (
                        data["given_names"]
                        if isinstance(data["given_names"], list)
                        else [data["given_names"]]
                    ),
                }
            ],
        ...
        if codigo_cbo:
            ...
            if cbo_service.validar_codigo(codigo_cbo):
                ...
                if ocupacao:
                    logger.info(
                        f"Practitioner created with CBO {codigo_cbo}: {ocupacao.nome}"
                    )
            else:
                return Response(
                    {
...
def list_practitioners(request):
    ...
    try:
        ...
        if specialty:
            ...
            practitioners = [
                p for p in practitioners if p.get("id") in practitioner_ids
            ]
                            ...
                            "coding": [
                                {
                                    ...
                                    "code": codigo_cbo,
                                    "display": (
                                        ocupacao.nome if ocupacao else codigo_cbo
                                    ),
                                },
                                {
                                    "system": "http://terminology.hl7.org/CodeSystem/v2-0360",
                                    ...
                                    "display": data.get(
                                        ...
                                        ocupacao.nome if ocupacao else "Doctor",
                                    ),
                                },
                            ],
                            "text": (
                                ocupacao.nome
                                if ocupacao
                                else data.get("qualification_display", "Médico")
                            ),
                        }
                    }
                ]
...
def create_practitioner_role(request):
    ...
    try:
        ...
        role = {
            ...
            "active": True,
            "practitioner": {"reference": f"Practitioner/{data['practitioner_id']}"},
        }
...
def list_specialties(request):
    ...
    try:
        ...
        return Response({"specialties": specialties, "total": len(specialties)})
...
@authentication_classes([KeycloakAuthentication])
@permission_classes([IsAuthenticated])
def get_or_update_practitioner(request, practitioner_id):
    """
    Get or Update practitioner by ID.

    GET /api/v1/practitioners/{id}/   → retorna o recurso Practitioner
    PUT /api/v1/practitioners/{id}/   → atualiza o recurso Practitioner
    """
    ...
    try:
        ...
        if data.get("prefix"):
            practitioner_resource["name"][0]["prefix"] = [data["prefix"]]
        if data.get("birthDate"):
            practitioner_resource["birthDate"] = data["birthDate"]
        telecom = []
        if data.get("phone"):
            telecom.append({"system": "phone", "value": data["phone"], "use": "work"})
        if data.get("email"):
