    ...
    return Response(
        {
            ...
            "sanitized_content": (
                BiasPreventionService.sanitize_content(content) if has_bias else content
            ),
        }
    )
...
def validate_recommendation(request):
    ...
    if not recommendation:
        return Response(
            {"error": "Recommendation is required"}, status=status.HTTP_400_BAD_REQUEST
        )

    result = BiasPreventionService.validate_clinical_recommendation(
        recommendation, patient_data
    )
...
def list_archetypes(request):
    ...
    return Response({"count": len(archetypes), "archetypes": archetypes})
...
def validate_archetype_data(request, archetype_name):
    ...
    http_status = (
        status.HTTP_200_OK if result["valid"] else status.HTTP_422_UNPROCESSABLE_ENTITY
    )
...
def map_to_fhir(request, archetype_name):
    ...
    try:
        fhir_resource = ISO13606ArchetypeService.map_to_fhir(
            archetype_name, data, patient_id
        )
...
def get_terminology_codes(request):
    ...
    return Response({"system": system.value, "count": len(codes), "codes": codes})
    ...
    return Response(
        {
            "compliance": {
                ...
                "data_anonymization": {
                    "status": "active",
                    "lgpd_compliant": True,
                    "pii_patterns_detected": list(
                        BiasPreventionService.PII_PATTERNS.keys()
                    ),
                },
