...
def list_pix_payments(request):
    ...
    return Response({"count": len(payments), "payments": payments})
...
def pix_webhook(request):
    ...
    try:
        ...
        if payment:
            return Response({"status": "confirmed", "payment": payment.to_dict()})
        else:
            return Response(
                {"error": "Payment not found"}, status=status.HTTP_404_NOT_FOUND
            )
...
def send_whatsapp_message(request):
    ...
    try:
        ...
        return Response(
            {"error": f"Invalid template: {e}"}, status=status.HTTP_400_BAD_REQUEST
        )
    except KeyError as e:
        return Response(
            {"error": f"Missing field: {e}"}, status=status.HTTP_400_BAD_REQUEST
...
def list_whatsapp_messages(request):
    ...
    return Response({"count": len(messages), "messages": messages})


@api_view(["GET", "POST"])
@permission_classes(
    [AllowAny]
)  # Webhook público - Meta verifica com token proprietário
...
def list_telemedicine_sessions(request):
    ...
    sessions = TelemedicineService.list_sessions(
        patient_id=patient_id, practitioner_id=practitioner_id, status=session_status
    )
